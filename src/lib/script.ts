    ...
    for (const id of beat.layers) {
      ...
      if (layer?.kind === 'motif' && layer.motif === 'crystal-viewer') {
        if (beat?.id === 'pore') {
          ...
          lines.push('Motif: ZS-9 — pore reveal reverses; framework and K⁺ return (drag to orbit)')
        ...
        lines.push(
          beat?.id === 'peri'
            ...
            : beat?.id === 'sky'
              ...
              : beat?.id === 'reveal'
                ? 'Motif: constellation nests into open drawer; zoom out, then pull back'
                : beat?.id === 'cabinets'
                  ? 'Motif: pull further into the aisle / full collection hall'
                  : beat?.id === 'instrument'
                    ? 'Motif: walk the cabinet row — drawers open as we pass'
                    : beat?.id === 'turn'
